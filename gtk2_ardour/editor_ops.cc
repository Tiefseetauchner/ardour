...
Editor::cursor_align (bool playhead_to_edit)
{
	...
	if (playhead_to_edit) {
		...
		_session->request_locate (selection->markers.front()->position().samples(), RollIfAppropriate);
...
Editor::jump_forward_to_mark ()
{
	...
	_session->request_locate (pos.samples(), RollIfAppropriate);
}
...
{
	if (entered_marker) {
		_session->request_locate (entered_marker->position().samples(), RollIfAppropriate);
	} else {
		samplepos_t sample;
		bool ignored;
		...
		if (_session) {
			_session->request_locate (where.samples(), RollIfAppropriate);
		}
	}
